...
exports.homepage = async (req, res) => {
  try {
    ...
    res.render("pages/home", {
      ...
      title: "Home",
      layout: "./layouts/main",
      joblist: joblist, // Pass the filtered joblist data to the 'home' template
    });
  } catch (error) {
    console.error(error);
...
exports.postStudentApp = async (req, res) => {
  ...
  try {
    ...
    if (applicantData) {
      ...
      applicantData.classNum = req.body.classNum;
  ...
  try {
    ...
    if (result) {
      ...
      if (req.user && req.user.role === 'admin') {
        ...
        // Redirect to the student dashboard for students
        res.redirect("/studentApp");
      }
    } else {
      console.log("Applicant not found");
      res.json({ success: false, error: "Applicant not found" });
